...
parser.add_argument('--n_layer', type=int, default=3,
                    help='number of functional layers')
...
parser.add_argument('--max_step', type=int, default=200000,
                    help='upper iteration limit.')
parser.add_argument('--global_batch_size', type=int, default=60,
                    help='global training batch size')
parser.add_argument('--global_eval_batch_size', type=int, default=16,
...
if rank == 0:
    ...
    work_dir = os.path.join("outputs", args.work_dir, model_path)
...
va_iter = corpus.get_iterator('valid', eval_batch_size, args.eval_tgt_len, **data_kwargs)
...
model = DEQTransformerLM(
        args, args.n_token, args.n_layer, args.n_head, args.d_model, args.d_head, args.d_inner, args.dropout, args.dropatt, 
        d_embed=args.d_embed, tgt_len=args.tgt_len, mem_len=args.mem_len, local_size=args.local_size, 
        tie_weights=args.tied, tie_projs=tie_projs, div_val=args.div_val, cutoffs=cutoffs, logging=logging
        )
...
scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, args.max_step, eta_min=args.lr_min)
...
def train():
    ...
    train_iter_steps = tr_iter.n_batch
    ...
    for batch, (data, target, seq_len) in tqdm(enumerate(train_iter), desc="Training", total=train_iter_steps, leave=False):
        ...
        if args.batch_chunk > 1:
            ...
            for i in range(args.batch_chunk):
                ...
                loss, mems[i], info = para_model(
                        data_i.t(), target_i.t(), mems[i]
                        )
            ...
            loss, mems, info = para_model(
                    data.t(), target.t(), mems
                    )
        ...
        if train_step % args.log_interval == 0:
            ...
            log_str = '| epoch {:3d} | step {:>8d} | {:>6d} batches | lr {:.3g} ' \
                ...
                epoch, train_step, batch+1, optimizer.param_groups[0]['lr'], elapsed * 1000 / args.log_interval, 
                cur_abs_error, cur_rel_error, cur_loss.item(), cur_ppl.item())
            logging(log_str)
        ...
        if train_step % args.eval_interval == 0:
            ...
            log_str = '| Eval {:3d} at step {:>8d} | time: {:5.2f}s ' \
                ...
                train_step // args.eval_interval, train_step, elapsed, 
                eval_abs_error.item(), eval_rel_error.item(), val_loss.item(), val_ppl.item())
            logging(log_str)
            logging('-' * 100)
...
if args.eval:
    ...
    log_str = '| Eval | time: {:5.2f}s ' \
            '| abs {:.3f} | rel {:.5f} | valid loss {:5.2f} | valid ppl {:9.3f}'.format(
            elapsed, eval_abs_error.item(), eval_rel_error.item(), val_loss.item(), val_ppl.item())
    logging(log_str)
    logging('-' * 100)
